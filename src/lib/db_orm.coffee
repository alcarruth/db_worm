#!/usr/bin/env coffee
#
#  web-worm/src/client/db_orm.coffee
# 
...
# General superclass for all column classes defined below.
# 
# 
class Column
  ...
  constructor: (spec) ->
    ...
    # Boolean sql_column indicates that this is a column
    # which can be accessed direcly from a row returned from
    # a database query.  It must be set to true or false in
    # a subclass.
    # 
...
# For a Local_Method the method is provided by the db_schema
# itself.  So that's what we return.
# 
...
# A Back_Reference is used to find rows in another table
# which refer back to the current row object.
# 
class Back_Reference extends Column
  ...
  # The foreign table name and the column name which might
  # refer to the current object are provided in the db_schema
  #   
  __column_method: =>
    { table_name, col_name } = @options
    return ->
...
#
# Methods toString() and toHTML() are defined here as defaults.  Table_Row
# should be extended and these methods overridden with app-specific methods
# producing the desired pretty strings and html code.
# 
class Table_Row
...
# CLASS DB_ORM
#
# Class DB_ORM provides an object-relational-mapping for a database which can be
# accesed by @db_obj and is described by @db_schema.
# 
class DB_ORM

  # TODO: perhaps move this up to column definitions at top of this file.
  # 
  # Dictionary column_Class maps db_schema column types to the apropropriate
  # column class defined above.
  # 
  column_Class:
    string: SQL_String
    integer: SQL_Integer
  ...
  # @db_obj: used to query db
  # @db_schema: used to interpret the response
  # 
  ...
  # add each table defined in db_schema
  init_tables: =>
    # @db_schema = @db_obj.db_schema
    @tables = {}
    @add_table(name, def) for name, def of @db_schema

  # add the table as defined in the db_schema
  add_table: (table_name, table_def) =>
    ...
    for col_name, col_def of table_def
      ...
      # There's just one key and value
      [type, options] = ([k,v] for k,v of col_def)[0]

      # TODO: there should be just one column with
      # primary_key set to true.  We could check for
      # that here but we don't so the last column with
      # primary_key true will become the primary key for
      # the table.
      # 
      primary_key = col_name if options.primary_key

      # create the appropriate column object for this
      # spec and add it to the columns dict.
      # 
    ...
    # creat the table object with the array of column objects
    # created above.
    # 
